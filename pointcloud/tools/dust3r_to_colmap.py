...
# 1. 取得絕對路徑
# 假設檔案在 /project2/yichuanh/GaMO/getmask/tools/dust3r_to_colmap.py
THIS_DIR = os.path.dirname(os.path.abspath(__file__))
# 向上跳兩層到 /project2/yichuanh/GaMO/
PROJ_ROOT = os.path.abspath(os.path.join(THIS_DIR, "..", ".."))

# 2. 定義你想要的目的地
GAMO_DIR = os.path.join(PROJ_ROOT, "gamo")         # 這是 dust3r 的父目錄
DUST3R_SRC = os.path.join(GAMO_DIR, "dust3r")      # 這是 dust3r 本身

# 3. 清理掉之前可能干擾的舊路徑 (選配，確保乾淨)
sys.path = [p for p in sys.path if "extern/dust3r" not in p]

# 4. 加入路徑
# 順序很重要：
# - 加入 GAMO_DIR 才能辨識 'import dust3r'
# - 加入 DUST3R_SRC 才能讓 dust3r 內部的相對 import 正常運作
for p in [DUST3R_SRC, GAMO_DIR]:
    if os.path.isdir(p):
        if p in sys.path:
            sys.path.remove(p)
        sys.path.insert(0, p)
...
# ----------------- 這一段是重點 -----------------
# 取得這支檔案所在的資料夾 (…/getmask/tools)
THIS_DIR = os.path.dirname(os.path.abspath(__file__))
# getmask 根目錄
REPO_ROOT = os.path.dirname(THIS_DIR)
# dust3r 的實際位置：…/getmask/third_party/ViewCrafter/extern/dust3r
DUST3R_DIR = os.path.join(REPO_ROOT, "third_party", "ViewCrafter", "extern", "dust3r")

# 安全加入 sys.path
for p in (REPO_ROOT, DUST3R_DIR):
