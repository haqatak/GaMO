...
class GaussianModel_tam:
    ...
    def prune(self, min_opacity, extent, max_screen_size, iter):
    ...
    def add_densification_stats_with_novel_pose(
            self, 
            viewspace_point_tensor, update_filter, 
            viewspace_point_tensor_novel_pose, update_filter_novel_pose, 
            viewspace_point_tensor_novel_pose_grad=None, 
            novel_pose_scale=1.):
        
        novel_pose_grad = viewspace_point_tensor_novel_pose.grad if viewspace_point_tensor_novel_pose_grad is None else viewspace_point_tensor_novel_pose_grad
        novel_pose_grad = novel_pose_grad / novel_pose_scale

        grad_both = viewspace_point_tensor.grad + novel_pose_grad
        filter_both = update_filter | update_filter_novel_pose
        self.xyz_gradient_accum[filter_both] += torch.norm(grad_both[filter_both, :2], dim=-1,
                                                             keepdim=True)
        self.denom[filter_both] += 1
    

    def add_points(self, new_pts, new_rgbs): 
        # new_pts/new_rgbs: [N, 3]
        new_pts = torch.from_numpy(new_pts).cuda()
        new_rgbs = torch.from_numpy(new_rgbs).cuda()
        n_pts = new_pts.shape[0]

        dist2 = torch.clamp_min(distCUDA2(new_pts)[0], 0.0000001)
        scales = torch.log(torch.sqrt(dist2))[..., None].repeat(1, 3)
        opacities = inverse_sigmoid(0.1 * torch.ones((n_pts, 1), dtype=torch.float, device="cuda"))

        _, d1, d2 = self._features_rest.shape
        _, d3 = self._rotation.shape

        new_xyz = new_pts
        new_scaling = scales
        new_rotation = torch.zeros((n_pts, d3)).float().cuda()
        new_features_dc = new_rgbs[:, None] # [N, 1, 3]
        new_features_rest = torch.zeros((n_pts, d1, d2)).float().cuda()
