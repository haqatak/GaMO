...
def training(dataset, opt, pipe, testing_iterations, saving_iterations, checkpoint_iterations, checkpoint, debug_from, total_iteration):
    ...
    if iteration in testing_iterations:
        ...
        for config in validation_configs:
            ...
            if cams and len(cams) > 0:
                ...
                # ========= 寫入 per_view.json（每張） =========
                # per_view_path = os.path.join(scene.model_path, "per_view.json")
                # if os.path.exists(per_view_path):
                #     try:
                #         with open(per_view_path, "r") as f:
                #             per_results = json.load(f)
                #     except json.JSONDecodeError:
                #         per_results = {}
                # else:
                #     per_results = {}

                # per_results.setdefault(str(iteration), {})
                # per_results[str(iteration)][config['name']] = per_view  # <-- per_view 是每張圖的dict

                # with open(per_view_path, "w") as f:
                #     json.dump(per_results, f, indent=4)
...
if __name__ == "__main__":
    ...
    parser.add_argument('--total_iteration', type=int, default=7000)
    #parser.add_argument("--test_iterations", nargs="+", type=int, default=[2_000, 3_000])#[3_000, 5_000, 7_000, 10_000, 15_000, 30_000])
    #parser.add_argument("--save_iterations", nargs="+", type=int, default=[2_000, 3_000])#[3_000, 5_000, 7_000, 10_000, 15_000, 30_000])
    
    parser.add_argument("--test_iterations", nargs="+", type=int, default=[3_000, 7_000, 10_000, 15_000, 30_000])#[3_000, 5_000, 7_000, 10_000, 15_000, 30_000])
    parser.add_argument("--save_iterations", nargs="+", type=int, default=[3_000, 7_000, 10_000, 15_000, 30_000]) #[3_000, 5_000, 7_000, 10_000, 15_000, 30_000])
    ...
    args.save_iterations.append(args.total_iteration)
    #breakpoint()
    ...
    training(lp.extract(args), op.extract(args), pp.extract(args), args.test_iterations, args.save_iterations, args.checkpoint_iterations, args.start_checkpoint, args.debug_from, args.total_iteration)
