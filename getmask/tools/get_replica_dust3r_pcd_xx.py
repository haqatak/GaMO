...
def main():
    ...
    for scene_name in args.scenes:
        ...
        # 2) 這邊要生出他原本要放在 dust3r_results 裡面那種「展開名字」
        #    原本你的寫法是：
        #    split = path.split("/")
        #    scene, seq = split[-2], split[-1]
        #    ...
        # 但你現在的 root 是 .../Replica_3_master/{scene}，底下**沒有 seq**，
        # 所以我這邊判斷一下：如果像 Replica 那種真的有 /office_2/seq_2 就拆，否則就用檔名本身。

        # 檢查有沒有再下一層 seq_x
        # e.g. /.../office_2/seq_2
        # 如果沒有，就用 scene_name 當 key
        # 你原本的 readColmapSceneInfo 會自己處理 dataset=Replica_3 這種未知情況
        # 所以這裡我們只要讓 dust3r 的輸出有個合理的名字就好
        # 這裡我就用 scene_name 當最後那層名字
        ...
        if args.demo:
            img_dir_for_dust3r = os.path.join(
                args.out_root,
                f"Replica_{args.n_views}v_thr{int(args.min_conf)}_trimeshsave_minconf1_demosetting",
                scene_key_for_save,
            )
        else:
            img_dir_for_dust3r = os.path.join(
                args.out_root,
                f"Replica_{args.n_views}v_thr{int(args.min_conf)}_trimeshsave_minconf1",
                scene_key_for_save,
            )
