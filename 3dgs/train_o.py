    ...
    for iteration in range(first_iter, opt.iterations + 1):
        ...
        with torch.no_grad():
            ...
            if (iteration in checkpoint_iterations):
                ...
                torch.save((gaussians.capture(), iteration), scene.model_path + "/chkpnt" + str(iteration) + ".pt")
"""
    ...
    if iteration in testing_iterations:
        ...
        for config in validation_configs:
            ...
            if cams and len(cams) > 0:
                ...
                if have_vgg:  per_view["LPIPS_VGG"]  = {}
                if have_alex: per_view["LPIPS_ALEX"] = {}
                ...
                with torch.no_grad():
                    for idx, viewpoint in enumerate(cams):
                        ...
                        # metrics（單張）
                        l1_v   = l1_loss(image, gt_image).mean().double()
                        psnr_v = psnr(image, gt_image).mean().double()
                        ssim_v = ssim(image.unsqueeze(0), gt_image.unsqueeze(0))
                        ...
                        psnr_avg += psnr_v
                        ssim_avg += ssim_v
                        if have_vgg  and s_vgg  is not None: lpips_vgg_avg  += s_vgg
                        if have_alex and s_alex is not None: lpips_alex_avg += s_alex

                        # ➜ 紀錄 per-view（以檔名為 key）
                        name = str(getattr(viewpoint, "image_name", f"idx_{idx}"))
                        per_view["L1"][name]   = float(l1_v)
                        per_view["PSNR"][name] = float(psnr_v)
                        per_view["SSIM"][name] = float(ssim_v)
                        if have_vgg  and s_vgg  is not None: per_view["LPIPS_VGG"][name]  = float(s_vgg)
                        if have_alex and s_alex is not None: per_view["LPIPS_ALEX"][name] = float(s_alex)
                ...
                if tb_writer:
                    ...
                    tb_writer.add_scalar(f"{config['name']}/loss_viewpoint - ssim", ssim_avg, iteration)
                    if have_vgg:
                ...
                results.setdefault(str(iteration), {})
                block = {
                    "L1": float(l1_avg),
                    "PSNR": float(psnr_avg),
                    "SSIM": float(ssim_avg),
                ...
                per_results.setdefault(str(iteration), {})
                per_results[str(iteration)][config['name']] = per_view
...
if __name__ == "__main__":
    ...
    parser.add_argument("--checkpoint_iterations", nargs="+", type=int, default=[1_0000])
